...
// designer text
firstText = "Turinio kūrėja";
secondText = "Marketingo specialistė";
thridText = "Freelancer'ė"
intervalTime = 600;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // SUSISIEKIME mygtuko logika
    const hireMeBtn = document.querySelector(".hire_me");
    if (hireMeBtn) {
        hireMeBtn.addEventListener("click", function () {
            // Nuimti tab-active nuo aktyvios sekcijos
            const activeTab = document.querySelector(".tabcontent.tab-active");
            if (activeTab) {
                activeTab.classList.remove("tab-active");
            }

            // Uždėti tab-active ant kontaktų sekcijos
            ...
            // Nuimti .current-tab nuo dabartinio meniu punkto
            const currentNavItem = document.querySelector(".icetab.current-tab");
            if (currentNavItem) {
                currentNavItem.classList.remove("current-tab");
            }

            // Uždėti .current-tab ant kontaktų meniu punkto
            const contactNavItem = Array.from(document.querySelectorAll(".icetab")).find(li => {
                return li.innerHTML.includes("contact_icon.svg");
            });

            if (contactNavItem) {
                contactNavItem.classList.add("current-tab");
            }
        });
    }
});
...
// Open overlay based on data-target
$(".pop-up").on("click", function () {
